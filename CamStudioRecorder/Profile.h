...
struct sXNoteOpts
{
    ...
    sXNoteOpts()
        ...
        , m_cXnoteDisplayFormatString("(0000)..00:00:00.000") // Default (Delay) hh:mm:ss.ttt, Not really a format. As
                                                              // long as the timer is not running this will be showed
...
// copy ala sVideoOpts
...
#else
        : m_taXNote(BOTTOM_LEFT)
        , m_cXnoteDisplayFormatString("(0000)..00:00:00.000") // Default hh:mm:ss.ttt, Not really a format. As long as
                                                              // the timer is not running this will be showed
    ...
    sXNoteOpts &operator=(const sXNoteOpts &rhs)
    {
        ...
        // Assign Xnote format string to 'Text atribute'.text (Required to get a time displayed on the capture without
        // pressing OK first in AnnotationEffects dialog.)
    ...
    {
        // XNote
        // TRACE("## bool Read(libconfig::Setting& cProfile\n");
        cProfile.lookupValue("Annotation", m_bAnnotation);
        if (cProfile.exists("TextAttributes"))
        {
    ...
    bool Write(libconfig::Setting &cProfile)
    {
        // TRACE("## bool Write(libconfig::Setting& cProfile\n");
        // TRACE("## ----------------------------------------------------------------------------\n");
        // TRACE("## m_bAnnotation : [%d]\n", m_bAnnotation   );
        // TRACE("## m_taXNote.text : [%s]\n", m_taXNote.text.GetString()   );
        // TRACE("## m_bXnoteRemoteControlMode: [%d]\n", m_bXnoteRemoteControlMode  );
        ...
        // Assign Xnote format string to .text because we need this to get a formatted print on the capture without
        // pressing OK first in AnnotationEffects dialog.
        m_taXNote.text = m_cXnoteDisplayFormatString;
        ...
        UpdateSetting(cProfile, "RemoteControl", m_bXnoteRemoteControlMode, libconfig::Setting::TypeBoolean);
        UpdateSetting(cProfile, "DisplayCameraDelay", m_bXnoteDisplayCameraDelayMode, libconfig::Setting::TypeBoolean);
        UpdateSetting(cProfile, "DisplayCameraDelayForwards", m_bXnoteDisplayCameraDelayDirection,
            libconfig::Setting::TypeBoolean);
        UpdateSetting(cProfile, "CameraDelayInMilliSec", (long &)m_ulXnoteCameraDelayInMilliSec, libconfig::Setting::TypeInt);
        std::string text(m_cXnoteDisplayFormatString);
        UpdateSetting(cProfile, "DisplayFormatString", text, libconfig::Setting::TypeString);

        UpdateSetting(cProfile, "RecordDurationLimitMode", m_bXnoteRecordDurationLimitMode, libconfig::Setting::TypeBoolean);
        UpdateSetting(cProfile, "RecordDurationLimitInMilliSec", (long &)m_ulXnoteRecordDurationLimitInMilliSec,
