...
void CXnoteStopwatchFormat::FormatXnoteDelayedTimeString(char *cBuf128, DWORD dwStartXnoteTickCounter,
                                                         DWORD dwCurrTickCount, long lDelayTimeInMillisSec,
                                                         bool bDisplayCameraDelay, bool bDisplayCameraDelay2)
{
    ...
    // JaHo: It appears that time event information from Xnote is delayed and Xnote time we have to write in the frame
    // should be added instead of subtracted. And to allow that we can define delaytime in two directions it must become
    // a long instead of an ulong in all code. (This is wrong direction) DWORD dwSubstractCorrTime =
    // dwStartXnoteTickCounter + lDelayTimeInMillisSec;
...
void CXnoteStopwatchFormat::FormatXnoteInfoSourceSensor(char *cBuf128, int iSourceInfo, int iSensorInfo)
{
    char cSourceTag = '.';
    char cSensorTag = '.';
