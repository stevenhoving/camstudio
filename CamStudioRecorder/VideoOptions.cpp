...
void CVideoOptionsDlg::DDV_CaptureInterval(CDataExchange *pDX, int value, int minVal, int maxVal)
{
    ...
    if (maxVal < value)
    {
        if (pDX->m_bSaveAndValidate)
        {
            if (!m_ctrlButtonAutoAdjust.GetCheck())
            {
                // MessageBox("Timelapse for each frame cannot be more than 7200000 milliseconds (2
                // hours).","Note",MB_OK);
    ...
    if ((value < minVal) || (maxVal < value))
    {
        if (pDX->m_bSaveAndValidate)
        {
            if (!m_ctrlButtonAutoAdjust.GetCheck())
            {
                ...
                MessageOut(*(pDX->m_pDlgWnd), IDS_STRING_PLAYBACKRATE, IDS_STRING_NOTE, MB_OK | MB_ICONEXCLAMATION,
                           value);
...
int CVideoOptionsDlg::CurrentLockPosition()
{
    ...
    {
        if (m_iCaptureInterval == m_arrCaptureRate[i] ||
            (m_iCaptureInterval <= m_arrCaptureRate[i] && m_iCaptureInterval >= m_arrCaptureRate[i + 1]))
        {
            pos = (i + 1) * 63;
            break;
        }
