...
#define EXPERIMENTAL_CODE
    ...
    {
        double interfloat =
            (((double)rAudioFormat.m_iInterleaveFactor) * ((double)cVideoOpts.m_iFramesPerSecond)) / 1000.0;
...
bool CamAVIFile::CompressionOpts(AVICOMPRESSOPTIONS &rOptions, CAVIStream &raviStream)
{
    ...
    if (bVideoStream)
    {
        ...
        rOptions.dwFlags = AVICOMPRESSF_VALID | AVICOMPRESSF_KEYFRAMES | AVICOMPRESSF_DATARATE;
        ...
        {
            double interfloat =
                (((double)m_sAudioFormat.m_iInterleaveFactor) * ((double)m_sVideoOpts.m_iFramesPerSecond)) / 1000.0;
...
bool CamAVIFile::Merge(const CString &strVideoIn, const CString &strAudioIn, const CString &strAVIOut)
{
    ...
    LPAVICOMPRESSOPTIONS lpAVICompressOptions[NUMSTREAMS] = {&AVICompressOptions[0], &AVICompressOptions[1]};
    // the editable streams
    PAVISTREAM AviStream[NUMSTREAMS];
    AviStream[0] = streamVideoEdit;
    AviStream[1] = streamAudioEdit;
    HRESULT hResult =
        streamVideoEdit.SaveV(strAVIOut, 0, AudioCompressCallBack, NUMSTREAMS, AviStream, lpAVICompressOptions);
    CAVI::OnError(hResult);
    bResult = (AVIERR_OK == hResult);
...
bool CamAVIFile::FadeOut(const CString &strAVIIn, const CString &strAVIOut)
{
    ...
    // count the keyframe.
    // Todo: GetInfo and NextKeyFrame are both memberfunction of CAVISTREAM and dwLength is indeed DWORD but
    // NextKeyFrame expect long instead of unsigned long
    ...
    for (long lPos = 0L; bResult && (lPos < (long)dwLength); lPos = streamIn.NextSample(lPos))
    {
        ...
        hResult = bResult && aviCompressedStreamOut.Write(lPos, 1L, (LPBYTE)cxImage.GetDIB() + lOffsetImage,
                                                          lpBitmap->biSizeImage, 0, 0, 0);
        CAVI::OnError(hResult);
        bResult = bResult && (AVIERR_OK == hResult);
