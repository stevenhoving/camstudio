...
bool CCamera::AddClicks(CDC *pDC)
{
    ...
    for (iter = queue.begin(); iter != queue.end(); ++iter)
    {
        ...
        {                    // once button is released we should remove all previous down events
            case 0xFFFFFFFF:
                continue; // <= WM_xBUTTONDOWN special case when button was released. Let it expire
            case WM_LBUTTONUP:
            case WM_MBUTTONUP:
            case WM_RBUTTONUP:
                iter2 = iter;
                ...
                {
                    --iter2;
                    if (iter2->flags == iter->flags - 1)
                    {                              // DOWN and UP are 1 apart
...
void CCamera::InsertText(CDC *pDC, const CRect &rectBase, TextAttributes &rTextAttrs)
{
    ...
    CFont *pOldFont = (font.m_hObject) ? dcBits.SelectObject(&font) : 0;
    ...
    CSize outerRectDimensions(sizeText.cx + 2 * nBorderLineThickness,
                              nNrOfLine * sizeText.cy + 2 * nBorderLineThickness); // Text and border
    CRect innerRectRelativePositons(nBorderLineThickness, nBorderLineThickness, sizeText.cx + nBorderLineThickness,
                                    nNrOfLine * sizeText.cy + nBorderLineThickness);
    ...
    pDC->BitBlt(outerRectOffset.cx, outerRectOffset.cy, outerRectDimensions.cx, outerRectDimensions.cy, &dcBits, 0, 0,
                SRCCOPY);
...
bool CCamera::CaptureFrame(const CRect &rectView)
{
    ...
    // m_rectFrame.left, m_rectFrame.bottom, m_rectFrame.right,  m_rectFrame.Width(), m_rectFrame.Height() );

    // setup DC's
    // Applied bug fix, tracker  ID: 3075791 / memory leak, reported and solved by mlt_msk, hScreenDC is released
    // afterwards.
    ...
    {
        cMemDC.BitBlt(0, 0, m_rectView.Width() + 1, m_rectView.Height() + 1, pScreenDC, m_rectView.left, m_rectView.top,
                      dwRop);
    }
    else
    {
