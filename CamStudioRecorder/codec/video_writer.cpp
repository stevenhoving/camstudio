    ...
    , meta_(meta)
{
    ...
    std::experimental::filesystem::path path(filename_);
...
int video_writer::write_frame(AVFormatContext *fmt_ctx, const AVRational *time_base, AVStream *st, AVPacket *pkt)
{
...
AVFrame * video_writer::alloc_audio_frame(enum AVSampleFormat sample_fmt, uint64_t channel_layout, int sample_rate, int nb_samples)
{
...
void video_writer::open_audio(AVFormatContext *oc, AVCodec *codec, output_stream *ost, AVDictionary *opt_arg)
{
    AVCodecContext *c;
    int nb_samples;
    int ret;
...
AVFrame * video_writer::get_audio_frame(output_stream *ost)
{
...
int video_writer::write_audio_frame(AVFormatContext *oc, output_stream *ost)
{
    AVCodecContext *c;
...
AVFrame * video_writer::alloc_picture(enum AVPixelFormat pix_fmt, int width, int height)
{
    AVFrame *picture;
    int ret;
...
void video_writer::open_video(AVFormatContext *oc, AVCodec *codec, output_stream *ost, AVDictionary *opt_arg)
{
    int ret;
...
void video_writer::close_stream(AVFormatContext *oc, output_stream *ost)
{
