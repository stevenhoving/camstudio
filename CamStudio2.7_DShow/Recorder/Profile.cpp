...
sAudioFormat& sAudioFormat::operator=(const sAudioFormat& rhs)
{
	...
	m_iAvgBytesPerSec		= rhs.m_iAvgBytesPerSec;
	m_iBlockAlign			= rhs.m_iBlockAlign;
	...
	m_iFormatTag			= rhs.m_iFormatTag;
	m_iCurrentCompression	= rhs.m_iCurrentCompression;
	m_sCaptureDevice		= rhs.m_sCaptureDevice;
	m_sCompressor			= rhs.m_sCompressor;
	DeleteAudio();
	if (rhs.m_pwfx && rhs.m_dwCbwFX) {
		if (NewAudio()) {
			...
			if (m_pwfx->wFormatTag != m_iFormatTag) {
				m_pwfx->wFormatTag = static_cast<WORD>(m_iFormatTag);
			}
		}
	} else {
...
bool sAudioFormat::WriteAudio(const LPWAVEFORMATEX pwfx)
{
	...
	m_iFormatTag = m_pwfx->wFormatTag;
