...
void FlashZLibBitmapData::Write(N_STD::ostream &out, char /*format*/)
{
    ...
    if (compress2(compression_buffer, &csize, (const unsigned char *)tmp.str().c_str(), tmp.str().size(),
...
void FlashZLibBitmapData::Read(N_STD::istream &in, char format, UDWORD input_buffer_size, UDWORD size_buffer)
{
    ...
    if (uncompress(compression_buffer, &csize, datatmp, input_buffer_size) == Z_OK)
    {
        if (format == 3)
        {
            ...
            {
                if (GetAlpha())
                    colors.push_back(FlashRGB(compression_buffer[it * 4], compression_buffer[it * 4] + 1,
                                              compression_buffer[it * 4] + 2, compression_buffer[it * 4] + 3));
                else
                    colors.push_back(FlashRGB(compression_buffer[it * 3], compression_buffer[it * 3] + 1,
                                              compression_buffer[it * 3] + 2));
            }
            if (GetAlpha())
                data = compression_buffer + sizec * 4;
            else
                data = compression_buffer + sizec * 3;
            len = len2;
        }
        else
        {
...
N_STD::ostream &operator<<(N_STD::ostream &out, FlashTagDefineBitsLossless &data)
{
    ...
    if (data.format == 3)
        tmp.put((unsigned char)data.d.GetSize() - 1);
...
N_STD::istream &operator>>(N_STD::istream &in, FlashTagDefineBitsLossless &data)
{
    ...
    if (data.format == 3)
        ...
        datasize = data.width * data.height * 4;
    // else throw;
