...
FlashButtonRecord::FlashButtonRecord(UWORD _charID, UWORD _depth, char _validstates, FlashMatrix &_matrix,
                                     FlashColorTransform &_cfx)
    : charID(_charID)
    , depth(_depth)
    , validstates(_validstates)
    , matrix(_matrix)
    , cfx(_cfx)
{
}
...
N_STD::ostream &operator<<(N_STD::ostream &out, FlashTagDefineButton &data)
{
    ...
    for (N_STD::vector<FlashButtonRecord *>::iterator ib = data.buttonrecords.begin(); ib != data.buttonrecords.end();
         ib++)
    {
        (*ib)->SetTagVersion(0);
        (*ib)->Write(tmp);
    ...
    for (N_STD::vector<FlashActionRecord *>::iterator ir = data.actionrecords.begin(); ir != data.actionrecords.end();
         ir++)
    {
        (*ir)->Write(tmp);
    }
...
N_STD::ostream &operator<<(N_STD::ostream &out, FlashTagDefineButton2 &data)
{
    ...
    for (N_STD::vector<flash_pair<N_STD::vector<FlashActionRecord *>, UWORD>>::iterator ir = data.actionrecords.begin();
         ir != data.actionrecords.end(); ir++)
    {
        N_STD::ostrstream tmp2;
        WRITE_UWORD2((*ir).second, tmp2);

        for (N_STD::vector<FlashActionRecord *>::iterator i = (*ir).first.begin(); i != (*ir).first.end(); i++)
        {
            (*i)->Write(tmp2);
        }
        tmp2.put((char)0);
