...
{
    unsigned char *output = (unsigned char *)buffer->buffer;
...
Buffer newBuffer()
{
    ...
    out->buffer = (char *)malloc(BUFFER_INCREMENT);
    out->pos = out->buffer;
    *(out->pos) = 0;
...
int bufferWriteData(Buffer b, sbyte *data, int length)
{
    int i;
...
int bufferWriteDataAndPush(Buffer a, Buffer b)
{
    ...
    if (a->pushloc && (b->buffer[0] == SWFACTION_PUSHDATA) && (b->pushloc == b->buffer + 1))
    ...
    while (p < out->pos)
    {
        if (*p & 0x80) /* then it's a multisbyte instruction */
        {
            if (*p == SWFACTION_BRANCHALWAYS)
            {
                ...
                if (*p == MAGIC_CONTINUE_NUMBER_LO && *(p + 1) == MAGIC_CONTINUE_NUMBER_HI)
                {
                    target = out->buffer - (p + 2);
                    *p = target & 0xff;
                    *(p + 1) = (target >> 8) & 0xff;
                }
                else if (*p == MAGIC_BREAK_NUMBER_LO && *(p + 1) == MAGIC_BREAK_NUMBER_HI)
                {
                    target = out->pos - (p + 2);
                    *p = target & 0xff;
                    *(p + 1) = (target >> 8) & 0xff;
...
int lookupSetProperty(char *string)
{
    ...
    if (strcmp(string, "y") == 0)
        return 0x3f80;
    if (strcmp(string, "xscale") == 0)
        return 0x4000;
    if (strcmp(string, "yscale") == 0)
        return 0x4040;
    ...
    if (strcmp(string, "name") == 0)
        return 0x4140;
    if (strcmp(string, "quality") == 0)
        return 0x4180;
    if (strcmp(string, "focusrect") == 0)
        return 0x4188;
    if (strcmp(string, "soundbuftime") == 0)
        return 0x4190;
...
char *lookupGetProperty(char *string)
{
    ...
    if (strcmp(string, "yscale") == 0)
        return "3";
    if (strcmp(string, "currentframe") == 0)
        return "4";
    if (strcmp(string, "totalframes") == 0)
        return "5";
    if (strcmp(string, "alpha") == 0)
        return "6";
    if (strcmp(string, "visible") == 0)
    ...
    if (strcmp(string, "target") == 0)
        return "11";
    if (strcmp(string, "framesloaded") == 0)
        return "12";
    if (strcmp(string, "name") == 0)
        return "13";
    if (strcmp(string, "droptarget") == 0)
        return "14";
