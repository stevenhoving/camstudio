...
{
    if (m.HasRotate())
        std::cout << "  Rotate:    " << (double)m.GetRotateX().lowerval / 0xffff + m.GetRotateX().upperval << ", "
                  << (double)m.GetRotateY().lowerval / 0xffff + m.GetRotateY().upperval << "\n";
    if (m.HasScale())
        std::cout << "  Scale:     " << (double)m.GetScaleX().lowerval / 0xffff + m.GetScaleX().upperval << ", "
                  << (double)m.GetScaleY().lowerval / 0xffff + m.GetScaleY().upperval << "\n";
...
void FlashMyImporter::PrintCFX(const FlashColorTransform &cfx)
{
    ...
    if (cfx.HasAdd())
    {
        ...
        {
            std::cout << "  AddRGBA:  " << cfx.GetAddRGB().GetR() << ", " << cfx.GetAddRGB().GetG() << ", "
                      << cfx.GetAddRGB().GetB() << ", " << cfx.GetAddRGB().GetA() << "\n";
        }
        else
            std::cout << "  AddRGB:   " << cfx.GetAddRGB().GetR() << ", " << cfx.GetAddRGB().GetG() << ", "
                      << cfx.GetAddRGB().GetB() << "\n";
    }
    if (cfx.HasMult())
    {
        ...
        {
            std::cout << "  MultRGBA:  " << cfx.GetMultRGB().GetR() << ", " << cfx.GetMultRGB().GetG() << ", "
                      << cfx.GetMultRGB().GetB() << ", " << cfx.GetMultRGB().GetA() << "\n";
        }
        else
            std::cout << "  MultRGB:   " << cfx.GetMultRGB().GetR() << ", " << cfx.GetMultRGB().GetG() << ", "
                      << cfx.GetMultRGB().GetB() << "\n";
    }
}
...
void FlashMyImporter::Import(FlashTagPlaceObject &data)
{
    ...
    if (data.HasColorTransform())
        ...
        std::cout << "HasColorTransform: false"
                  << "\n";
}
void FlashMyImporter::Import(FlashTagPlaceObject2 &data)
{
    ...
    if (data.HasCharID())
        std::cout << "HasCharID: true [" << data.GetCharID() << "]\n";
    else
        std::cout << "HasCharID: false"
                  << "\n";
    if (data.HasMatrix())
    {
    ...
    if (data.HasColorTransform())
    {
        ...
        std::cout << "HasColorTransform: false"
                  << "\n";
    if (data.HasMove())
        std::cout << "HasMove: true"
                  << "\n";
    else
        std::cout << "HasMove: false"
                  << "\n";
    if (data.HasRatio())
        std::cout << "HasRatio: true [" << data.GetRatio() << "]\n";
    else
        std::cout << "HasRatio: false"
                  << "\n";
    if (data.HasName())
        std::cout << "HasName: true [" << data.GetName() << "]\n";
    else
        std::cout << "HasName: false"
                  << "\n";
    if (data.HasClipDepth())
        std::cout << "HasClipDepth: true [" << data.GetClipDepth() << "]\n";
    else
        std::cout << "HasClipDepth: false"
                  << "\n";
}
...
void FlashMyImporter::Import(FlashTagStartSound &data)
{
    ...
    if ((data.GetSoundInfo().GetFlags() & FSI_HAS_IN) != 0)
        std::cout << "  In Point: " << data.GetSoundInfo().GetInPoint() << "\n";
    if ((data.GetSoundInfo().GetFlags() & FSI_HAS_OUT) != 0)
        std::cout << "  Out Point: " << data.GetSoundInfo().GetOutPoint() << "\n";
    if ((data.GetSoundInfo().GetFlags() & FSI_HAS_LOOP) != 0)
        std::cout << "  Loop Count: " << data.GetSoundInfo().GetLoopCount() << "\n";
    if ((data.GetSoundInfo().GetFlags() & FSI_HAS_ENVELOPE) != 0)
        std::cout << "  Sound Envelopes Found\n";
}
...
bool FlashImporter::ImportTag(N_STD::istream &in, FlashImportHandler &i)
{
    ...
    else IMPORT_MACRO(26, FlashTagPlaceObject2) else IMPORT_MACRO(
        5,
        FlashTagRemoveObject) else IMPORT_MACRO(28,
                                                FlashTagRemoveObject2) else IMPORT_MACRO(1,
                                                                                         FlashTagShowFrame) else IMPORT_MACRO(9,
                                                                                                                              FlashTagBackgroundColor) else IMPORT_MACRO(43,
                                                                                                                                                                         FlashTagLabelFrame) else IMPORT_MACRO(24,
                                                                                                                                                                                                               FlashTagProtect) else IMPORT_MACRO(0, FlashTagEnd) else IMPORT_MACRO(56, FlashTagExportAssets) else IMPORT_MACRO(57, FlashTagImportAssets) else IMPORT_MACRO(8, FlashTagJPEGTables) else IMPORT_MACRO(6,
                                                                                                                                                                                                                                                                                                                                                                                                                     FlashTagDefineBitsJPEG1) else IMPORT_MACRO(21, FlashTagDefineBitsJPEG2) else IMPORT_MACRO(35,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               FlashTagDefineBitsJPEG3) else IMPORT_MACRO(20,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          FlashTagDefineBitsLossless) else IMPORT_MACRO(36,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        FlashTagDefineBitsLossless2)

        else IMPORT_MACRO(2, FlashTagDefineShape1) else IMPORT_MACRO(22, FlashTagDefineShape2) else IMPORT_MACRO(
            32,
            FlashTagDefineShape3) else IMPORT_MACRO(46,
                                                    FlashTagDefineMorphShape) else IMPORT_MACRO(3,
    ...
    else IMPORT_MACRO(15, FlashTagStartSound) else IMPORT_MACRO(18, FlashTagSoundStreamHead) else IMPORT_MACRO(
        45, FlashTagSoundStreamHead2) else IMPORT_MACRO(19, FlashTagSoundStreamBlock)

        else IMPORT_MACRO(12, FlashTagDoAction) else IMPORT_MACRO(7, FlashTagDefineButton) else IMPORT_MACRO(
            34, FlashTagDefineButton2) else IMPORT_MACRO(17, FlashTagDefineButtonSound)
...
bool FlashImporter::ImportTag_AT(N_STD::istream &in, FlashImportHandler &i)
{
    ...
    else IMPORT_MACRO_AT(26, FlashTagPlaceObject2) else IMPORT_MACRO_AT(5, FlashTagRemoveObject) else IMPORT_MACRO_AT(28, FlashTagRemoveObject2) else IMPORT_MACRO_AT(1, FlashTagShowFrame) else IMPORT_MACRO_AT(
        9,
        FlashTagBackgroundColor) else IMPORT_MACRO_AT(43, FlashTagLabelFrame) else IMPORT_MACRO_AT(24, FlashTagProtect) else IMPORT_MACRO_AT(0, FlashTagEnd) else IMPORT_MACRO_AT(56, FlashTagExportAssets) else IMPORT_MACRO_AT(57, FlashTagImportAssets) else IMPORT_MACRO_AT(8, FlashTagJPEGTables) else IMPORT_MACRO_AT(6,
                                                                                                                                                                                                                                                                                                                            FlashTagDefineBitsJPEG1) else IMPORT_MACRO_AT(21,
                                                                                                                                                                                                                                                                                                                                                                          FlashTagDefineBitsJPEG2) else IMPORT_MACRO_AT(35,
                                                                                                                                                                                                                                                                                                                                                                                                                        FlashTagDefineBitsJPEG3) else IMPORT_MACRO_AT(20,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                      FlashTagDefineBitsLossless) else IMPORT_MACRO_AT(36,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       FlashTagDefineBitsLossless2)

        else IMPORT_MACRO_AT(2, FlashTagDefineShape1) else IMPORT_MACRO_AT(22, FlashTagDefineShape2) else IMPORT_MACRO_AT(
            32,
            FlashTagDefineShape3) else IMPORT_MACRO_AT(46,
                                                       FlashTagDefineMorphShape) else IMPORT_MACRO_AT(3,
                                                                                                      FlashTagFreeCharacter)
    ...
    else IMPORT_MACRO_AT(15, FlashTagStartSound) else IMPORT_MACRO_AT(18, FlashTagSoundStreamHead) else IMPORT_MACRO_AT(
        45, FlashTagSoundStreamHead2) else IMPORT_MACRO_AT(19, FlashTagSoundStreamBlock)

        else IMPORT_MACRO_AT(12, FlashTagDoAction) else IMPORT_MACRO_AT(7, FlashTagDefineButton) else IMPORT_MACRO_AT(
            34, FlashTagDefineButton2) else IMPORT_MACRO_AT(17, FlashTagDefineButtonSound)
