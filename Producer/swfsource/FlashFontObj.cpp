...
bool FlashFontFactory::GetGlyphShape(const char *font, UWORD charindex, FlashShape &s, bool bold, bool italic,
                                     bool uLine)

{
    ...
    HFONT myfont =
        CreateFont(-base, 0, 0, 0, fWeight, italic ? TRUE : FALSE, uLine ? TRUE : FALSE, FALSE, DEFAULT_CHARSET,
...
int FlashFontFactory::GetGlyphAdvance(const char *fontname, UWORD charindex, int pointsize)
{
    ...
    HFONT myfont = CreateFont(-1300 / 50 * pointsize, 0, 0, 0, FW_REGULAR, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
...
UWORD FlashFontFactory::WriteText(std::ostream &out, const char *fontname, const char *text, int x, int y,
                                  FlashRGB color, int pointsize, int depth, int extraspacing, bool bold, bool italic,
                                  bool uLine)
{
    ...
    for (int i = 0; text[i] != 0;)
    {
        ...
        if (shapes.find(uCode) == shapes.end())
        {
            ...
            if (!GetGlyphShape(fontname, (UWORD)uCode, shapes[uCode], bold, italic, uLine))
                return false;
        ...
        FlashRect b(r.GetX1() / 50 * pointsize + width + x + extraspacing * i, r.GetY1() / 50 * pointsize + y,
                    r.GetX2() / 50 * pointsize + width + x + extraspacing * i, r.GetY2() / 50 * pointsize + y);
        ...
        if (bDBCS)
            ...
            i++;
        textBounds.SetRect(x, y, textBounds.GetX2() + r.GetX2() - r.GetX1(),
...
UWORD FlashFontFactory::WriteText(std::ostream &out, const char *fontname, const char *text, int x, int y,
                                  FlashRGB color, int pointsize, int depth, FlashRect &textBounds, int extraspacing,
                                  bool bold, bool italic, bool uLine)
{
    stdext::hash_map<int, FlashShape> shapes;
    stdext::hash_map<int, int> shapes_lookup;
...
UWORD FlashFontFactory::GetBounds(std::ostream &out, const char *fontname, const char *text, int x, int y,
                                  FlashRGB color, int pointsize, int /*depth*/, FlashRect &textBounds, int extraspacing,
                                  bool bold, bool italic, bool uLine)
{
    stdext::hash_map<int, FlashShape> shapes;
