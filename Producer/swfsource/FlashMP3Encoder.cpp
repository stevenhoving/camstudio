...
int FlashMP3Encoder::SeekToNextValidHeader(FILE *fp, int &layer, int &ver, int &freq, int &stereo, int &rate)
{
    unsigned char hdr[4];
...
void FlashMP3Encoder::WriteStreamBlock(N_STD::ostream &out)
{
    ...
    if (!m_done)
    {
        ...
        for (;;)
        {
            if ((double)mspf_mp3 * i + (m_delay * mspf_mp3 / pcmFrameSize) > mspf)
                break;
            i++;
        }
        long size = 0;
        ...
        out << FlashTagSoundStreamBlockMP3((UWORD)pcmFrameSize * index, (UWORD)m_delay, (char *)out2,
                                           (UDWORD)size); // 1152*index,

        m_frame++;
        m_pos += index;
        m_delay = (long)(((m_pos * mspf_mp3) - (m_frame * mspf)) * pcmFrameSize / mspf_mp3);

        if (m_pos >= m_frameData.size())
            m_done = true;
    }
}
...
UWORD FlashMP3Encoder::WriteDefineTag(N_STD::ostream &out, int characterId)
{
    ...
    FlashTagDefineSoundMP3 tag((samplerate << 2) | (1 << 1) | (m_stereo), (UDWORD)pcmFrameSize * m_sizes.size(), data,
                               (UDWORD)len, (UDWORD)0);
    if (characterId != 0)
    {
