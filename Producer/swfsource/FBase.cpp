...
FlashFixed::FlashFixed(void)
    : upperval(0)
    , lowerval(0)
{
}

FlashFixed::FlashFixed(SWORD u, UWORD l)
    : upperval(u)
    , lowerval(l)
{
}

FlashFixed::FlashFixed(double f)
    : upperval((UWORD)floor(f))
    , lowerval(UWORD((f - floor(f)) * 0x10000))
{
}
...
FlashFixed16::FlashFixed16(SBYTE u, UBYTE l)
    : upperval(u)
    , lowerval(l)
{
}
...
FlashFixed16::FlashFixed16(double f)
    : upperval((UBYTE)floor(f))
    , lowerval(UBYTE((f - floor(f)) * 0x100))
{
}
...
N_STD::ostream &operator<<(N_STD::ostream &out, const FlashMatrix &data)
{
    ...
    {
        int bits =
            fbase_max(GetBitSizeSigned(data.scalex.ConvertToRaw()), GetBitSizeSigned(data.scaley.ConvertToRaw()));
        b.Write(bits, 5);
    ...
    {
        int bits =
            fbase_max(GetBitSizeSigned(data.rotatex.ConvertToRaw()), GetBitSizeSigned(data.rotatey.ConvertToRaw()));
        b.Write(bits, 5);
        b.Write(PackBitsSigned(data.rotatex.ConvertToRaw()), bits);
        b.Write(PackBitsSigned(data.rotatey.ConvertToRaw()), bits);
    }

    int bits = fbase_max(GetBitSizeSigned(data.translatex), GetBitSizeSigned(data.translatey));
    b.Write(bits, 5);
...
void FlashRGB::Write(BitBuffer &out, int num)
{
    if (num != -1)
    {
        ...
        if (alpha)
            out.Write(PackBitsSigned(a), num);
    }
    else
    {
        out.Write(r, 8);
...
N_STD::ostream &operator<<(N_STD::ostream &out, const FlashRGB &data)
{
    ...
    if (data.alpha)
        out.put((data.a < 0xff) ? ((char)data.a) : (char)0xff);
    return out;
}
...
BitStreamOut &operator<<(BitStreamOut &out, BitBuffer &data)
{
    ...
    for (N_STD::vector<char>::iterator i = data.v.begin(); i != data.v.end(); i++)
    {
        ...
        if (data.displace - displacement > 8)
        {
            ...
            out.Write((char)*i, data.displace - displacement);
    }
    return out;
}
...
N_STD::ostream &operator<<(N_STD::ostream &out, const FlashRect &data)
{
    int minbits = fbase_max(fbase_max(GetBitSize(data.x1), GetBitSize(data.y1)),
                            fbase_max(GetBitSize(data.x2), GetBitSize(data.y2))) +
                  1;
...
{
    int minbits =
        (int)ceil(((fbase_max(GetBitSize(data.size.GetX2()), GetBitSize(data.size.GetY2()) + 1)) * 4 + 5) / 8.0);
...
N_STD::ostream &operator<<(N_STD::ostream &out, const FlashTag &data)
{
    return out;
}
N_STD::istream &operator>>(N_STD::istream &in, FlashTag &data)
{
    return in;
}
...
FlashMatrix CreateMatrix(FlashRect bounds, float scaleX, float scaleY, float rotation, float translateX,
                         float translateY, bool bScale, bool bRotate)
{
    ...
    */
    ...
    return FlashMatrix(bScale, FlashFixed(a), FlashFixed(d), bRotate, FlashFixed(b), FlashFixed(c),

                       //                        translateX - int( deltaX * sX ),
                       //                        translateY - int( deltaY * sY )

                       // By Yiyi
                       (int)(translateX + centerX - int(deltaX * sX)), (int)(translateY + centerY - int(deltaY * sY)));
}
