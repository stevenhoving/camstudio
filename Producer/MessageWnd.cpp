...
HBITMAP CTransparentWnd::DrawResampleRGB(CDC *thisDC, CRect * /*caprect*/, int factor, LPBITMAPINFOHEADER expanded_bmi)
{
    ...
    (void)StretchDIBits(thisDC->m_hDC, 0, 0, reduced_width, reduced_height, 0, 0, reduced_width, reduced_height, lpBits,
                        (LPBITMAPINFO)smallbi, DIB_RGB_COLORS, SRCCOPY);
...
HANDLE AllocMakeDib(int reduced_width, int reduced_height, UINT bits)
{
    ...
    dwSize = sizeof(BITMAPINFOHEADER) + wColSize + (DWORD)(UINT)wLineLen * (DWORD)(UINT)reduced_height;
