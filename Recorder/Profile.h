...
struct sVideoOpts
{
	sVideoOpts()
		...
		, m_sCompressorName("Microsoft Video 1")
		, m_sCaptureFilterName("CamStudio Screen Capture")
	{
	}
	...
	sVideoOpts& operator=(const sVideoOpts& rhs)
	{
		...
		m_sCompressorName			= rhs.m_sCompressorName;
		m_sCaptureFilterName		= rhs.m_sCaptureFilterName;
	...
	bool Read(Setting& cProfile)
	{
		...
		cProfile.lookupValue("CompressorName", m_sCompressorName);
		cProfile.lookupValue("CaptureFilter", m_sCaptureFilterName);
		State(dwSize);
	...
	bool Write(Setting& cProfile)
	{
		...
		UpdateSetting(cProfile,"CompressorName", m_sCompressorName, Setting::TypeString);
		UpdateSetting(cProfile,"CaptureFilter", m_sCaptureFilterName, Setting::TypeString);
		return true;
	}
	...
	CString m_cStartRecordingString;
	std::string m_sCompressorName;
	std::string m_sCaptureFilterName;
protected:
	LPVOID m_pState;
...
public:
	sAudioFormat()
		...
		, m_iInterleavePeriod(CS_MILLISECONDS)
		, m_iMixerDevices(0)
		, m_iSelectedMixer(0)
		, m_iFeedbackLine(0)
	...
	bool Read(Setting& cProfile)
	{
		...
		cProfile.lookupValue("BitsPerSample", m_iBitsPerSample);
		cProfile.lookupValue("Channels", m_iNumChannels);
		cProfile.lookupValue("SamplesPerSecond", m_iSamplesPerSeconds);
		cProfile.lookupValue("AvgSamplesPerSec", m_iAvgBytesPerSec);
		cProfile.lookupValue("BlockAlign", m_iBlockAlign);
		cProfile.lookupValue("NumDev", m_iMixerDevices);		
		cProfile.lookupValue("SelectedDev", m_iSelectedMixer);
		cProfile.lookupValue("CompressionFormatTag", m_iFormatTag);
		AudioFormat().wFormatTag = static_cast<WORD>(m_iFormatTag);
		...
		cProfile.lookupValue("AudioDevice", m_sCaptureDevice);
		cProfile.lookupValue("Compressor", m_sCompressor);
		cProfile.lookupValue("CurrentCompressionFormat", m_iCurrentCompression);
		return true;
	}
	bool Write(Setting& cProfile)
	{
		...
		UpdateSetting(cProfile,"BitsPerSample", m_iBitsPerSample,Setting::TypeInt);
		UpdateSetting(cProfile,"Channels", m_iNumChannels,Setting::TypeInt);
		UpdateSetting(cProfile,"SamplesPerSecond", m_iSamplesPerSeconds,Setting::TypeInt);
		UpdateSetting(cProfile, "AvgSamplesPerSec", m_iAvgBytesPerSec, Setting::TypeInt);
		UpdateSetting(cProfile, "BlockAlign", m_iBlockAlign, Setting::TypeInt);
		...
		m_iFormatTag = AudioFormat().wFormatTag;
		UpdateSetting(cProfile,"CompressionFormatTag", m_iFormatTag,Setting::TypeInt);
		...
		UpdateSetting(cProfile,"AudioDevice", m_sCaptureDevice,Setting::TypeString);
		UpdateSetting(cProfile,"Compressor", m_sCompressor,Setting::TypeString);
		UpdateSetting(cProfile, "CurrentCompressionFormat", m_iCurrentCompression, Setting::TypeInt);
		return true;
	}
	...
	std::string m_sCaptureDevice;
	std::string m_sCompressor;
private:
	bool NewAudio();
