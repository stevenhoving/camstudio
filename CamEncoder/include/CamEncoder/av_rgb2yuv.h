...
void bgr2yuv420p_v2(uint8_t *destination[8], const uint8_t *const src[3], const int width, const int height, const int src_stride[3])
{
    ...
    for (int line = 0; line < height; ++line)
    {
        ...
        {
            int x = 0;
            for (x = 0; x < sse_aligned_width; x += 24)
            {
                ...
                const auto pxl4 = _mm_lddqu_si128((__m128i *)bgr_pixels); // load 4 pixels
                bgr_pixels += 4;

                const auto pxl5 = _mm_lddqu_si128((__m128i *)bgr_pixels); // load 4 pixels
                bgr_pixels += 4;

                // unpack so we end up with 6x 4 pixels
                ...
                auto vec_data4 = vec3_unpack_lo_bgr_odd(pxl4);
                auto vec_data5 = vec3_unpack_hi_bgr_odd(pxl5);

                // interleave so we end up with 3x 8 pixels
                auto vec_part0 = vec3_interleave(vec_data0, vec_data1);
                auto vec_part1 = vec3_interleave(vec_data2, vec_data3);
                auto vec_part2 = vec3_interleave(vec_data4, vec_data5);

                // multiply the 3x 8 pixels
                auto vec_y_part0 = vec3_mullo(vec_part0, y_mul);
                auto vec_y_part1 = vec3_mullo(vec_part1, y_mul);
                auto vec_y_part2 = vec3_mullo(vec_part2, y_mul);

                // vertical sum the vec3 so we end up with 1 object that contains 3x 8 pixels.
                auto vec_y_part = vec3_vsum(vec_y_part0, vec_y_part1, vec_y_part2);

                auto vec_result = vec3_shuffle(vec_y_part, y_shuffle_lo_odd);

                auto y_result0 = _mm_or_si128(vec_result.b, vec_result.g);
                auto y_result1 = vec_result.r;

                y_result0 = _mm_add_epi8(y_result0, y_add);
                y_result1 = _mm_add_epi8(y_result1, y_add);

                // store 24 y pixels
                _mm_store_si128((__m128i *)y, y_result0); y += 16;
                //_mm_store_si128((__m128i *)y, y_result1); y += 8;
                _mm_storel_epi64((__m128i *)y, y_result1); y += 8;
                ...
                if (x % 2 == 0)
                {
                    ...
                    auto u_vec_part2 = vec3_mullo(vec_part2, u_mul); // contains 4 pixels
                    ...
                    auto v_vec_part2 = vec3_mullo(vec_part2, v_mul); // contains 4 pixels

                    auto u_part0 = vec3_vsum(u_vec_part0, u_vec_part1, u_vec_part2);
                    auto v_part0 = vec3_vsum(v_vec_part0, v_vec_part1, v_vec_part2);

                    u_part0 = vec3_srai(u_part0, 8);
                    v_part0 = vec3_srai(v_part0, 8);

                    u_part0 = vec3_add(u_part0, uv_add);
                    v_part0 = vec3_add(v_part0, uv_add);

                    u_part0 = vec3_shuffle(u_part0, uv_shuffle);
                    v_part0 = vec3_shuffle(v_part0, uv_shuffle);

                    auto u_0 = vec3_pack_interleave(u_part0);
                    auto v_0 = vec3_pack_interleave(v_part0);

                    _mm_store_si128((__m128i *)u, u_0); u += 12;
                    _mm_store_si128((__m128i *)v, v_0); v += 12;
                }
            }
